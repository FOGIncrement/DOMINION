 ...
 * (food) below. Mutates the winning company's stocks/cash on the same
 * CompanySnapshot object already sitting in the tick's `companies` array;
 * never writes to the DB. That array is loaded once at the top of
 * runTick(), before the settlement loop runs — the company loop (which runs
 * after every settlement is already persisted) writes each company with an
 * absolute value computed from that same object, so a DB write made here
 * would just be silently clobbered a moment later. Mutating in place means
 * that later write picks up the change automatically (verified against
 * tickCompany, which reads company.stocks/cash straight off the snapshot
 * it's given).
 */
export function buyFromOwnedCompany(
  ...
  const company = [...candidates].sort((a, b) => (b.stocks[resource] ?? 0) - (a.stocks[resource] ?? 0))[0];
  const stock = company.stocks[resource] ?? 0;
  if (stock <= 0.01) return null;

  const affordable = goldBudget / unitPrice;
  const quantity = Math.max(0, Math.min(desiredQuantity, affordable, stock));
  if (quantity <= 0.01) return null;

  const revenue = quantity * unitPrice;
  company.stocks[resource] = stock - quantity;
...
export function maybeBuyFromOwnedRetail(
  ...
  const sale = buyFromOwnedCompany(retailCompanies, "food", need, state.gold, price);
  if (sale) {
 ...
 * on "goods" from the open market for a happiness boost beyond plain food
 * sufficiency. Used to prefer a Bakery this settlement's own player owns
 * before falling back to the market — Bakery no longer produces "goods"
 * (it produces "bread" now, see the recipe-economy plan; wiring new named
 * products into population consumption/happiness is deferred), so this is
 * market-only until that follow-up lands. No Settlement.goods field exists
 * or is added: goods bought this way are consumed for happiness the instant
 * they're bought, never stockpiled.
 */
export async function applyLuxuryGoodsPurchase(
  ...
  let fulfilled = 0;
  let spent = 0;
  const marketQuantity = Math.max(0, Math.min(desired, goldBudget / price));
  ...
  return { happinessBoost, sale: null };
}
